...
module Serializable_class = struct
  ...
  class ['a] serializable_from_json_capable fromjson tojson = object
    method from_encoding (b:string) : 'a =
  ...
  class ['a] serializable_from_json_capable_version_wrapped fromjson tojson = object(self)
    inherit ['a] serializable_from_json_capable fromjson tojson as super
    method private serialize_version version to_json (v:'a) : string =
      match version with
      | 0 -> let j = to_json v in j
      | n -> failwith("Unknown serializer protocol version: " ^ (string_of_int n))

    method private serialize_proto (a_to_json : 'a -> string) (v:'a) : string =
      let versioned =
	{Versioned.version = Versioned.proto_version; 
	 data = self#serialize_version Versioned.proto_version a_to_json v} in
      Yojson.Safe.to_string (Versioned.to_yojson versioned)

    method private deserialize_version version (a_of_json : string -> 'a) (b:string) =
      match version with 
      | 0 -> a_of_json b
      | n -> failwith("Unknown deserializer protocol version: " ^ (string_of_int n) ^ "\n")

    method private deserialize_proto (a_of_json : string -> 'a) (b:string) : 'a =
      let open Versioned in
      let versioned_result = Versioned.of_yojson (Yojson.Safe.from_string b) in
      match versioned_result with
      | `Ok versioned -> self#deserialize_version versioned.version a_of_json versioned.data
      | `Error s -> failwith(s)

    method from_encoding (b:string) : 'a =
      let result = self#deserialize_proto super#from_encoding b in result
    ...
    method to_encoding (v:'a) : string =
      self#serialize_proto super#to_encoding v
			      ...
			      (*
module Serializable_class_deprecated = struct
