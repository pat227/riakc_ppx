...
module Make_serializable_from_protobuf_capable(M:Protobuf_capable) = 
  struct
    type t = M.t
    let from_encoding (b:bytes) =
      let d = Protobuf.Decoder.of_bytes b in
      M.from_protobuf d;;
    let to_encoding t =
      let e = Protobuf.Encoder.create () in 
      M.to_protobuf t e;
      Bytes.of_string(Protobuf.Encoder.to_string e);;
  end

module Make_serializable_from_yojson_capable(M:Json_capable) = 
  struct
    type t = M.t
    let from_encoding (b:bytes)  =
      let open Yojson in
